...
In sheets, things with red outlines signify a clickable function, while blue signifies a double-clickable. Single-clicks are usually some function, like opening the dice pool dialog, whereas double-clickable is usually for outputting info to the chat about the thing. Double-clicking on weapons sends an info message to chat with buttons for both attacking with the weapon as well as defending against the weapon as the current actor. Rolling the defense first will allow the attacker to right click the opposing roll and select a context option to attack against the defense directly, with the attack roll automatically taking the right values from the defense roll. Weapon attack damage messages also have soak buttons for different damage levels that allow quickly rolling soak against the weapon as the current actor.  
...
Most buttons in the sheet or the chat that perform rolls can be "quickrolled" by holding a configurable keybind down, default *Control*. While not applicable with every button, most allow the popup dialog to be skipped and the roll or function performed with the default given values. Normal attack rolls are a big exception. (**Warning:** As of 9.245 there's a core bug where the Foundry keyboard system gets "stuck" if you hold down a key while clicking on the chat buttons. Switching to another window and back resets the system but it can get stuck again.)  

...
While currently not used in the programming, the *Effect* field in weapons should be formatted so that every attribute is separated with a comma, eg. "Damage +2, Slaying, Awkward", for future-proofing. Weapons with special resist effects should not add it to the effects field and instead add the resisting stats into the "Resist with" field.  
